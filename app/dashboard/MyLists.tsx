...
export default function MyLists({
  ...
  const renderListItem = (item: MediaStatusWithMedia, listStatus: StatusKey) => {
    ...
    switch (item.status) {
      case "TO_WATCH": actionButtons = ( <div className="flex flex-col sm:flex-row gap-1"> <ActionButton label="Começar" colorClass="bg-blue-600" disabled={isUpdating} onClick={() => onUpdateStatus(item, "WATCHING")} /> <ActionButton label="Já Vi" colorClass="bg-gray-500" disabled={isUpdating} onClick={() => onUpdateStatus(item, "WATCHED")} /> <ActionButton label="Abandonar" colorClass="bg-red-600" disabled={isUpdating} onClick={() => onUpdateStatus(item, "DROPPED")} /> </div> ); break;
      case "WATCHING":
        actionButtons = (
